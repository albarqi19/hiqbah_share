...
export const MODULE_SUB_PRIVILEGES: Record<string, { key: string; label: string }[]> = {
  ...
  stations: [
    { key: "claim", label: "Claim / release station queue items" },
    { key: "advance", label: "Advance station queue items" },
    { key: "supervisor_override", label: "Force-release another operator's claim" },
  ],
};
